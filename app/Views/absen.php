...
<div class="row">
  ...
  <button type="button" id="absenPulangButton" onclick="captureimage('0')" class="btn btn-warning" disabled>Pulang</button>
<?php endif ?>
  ...
  <script type="text/javascript">
    ...
    document.addEventListener("DOMContentLoaded", function() {
      const absenButton = document.getElementById("absenPulangButton"); // Pastikan ID tombol sesuai
      absenButton.addEventListener("click", function(event) {
        const now = new Date();
        const currentHour = now.getHours();

        if (currentHour < 16) {
          event.preventDefault(); // Mencegah form submit
          alert("Tidak bisa absen pulang sebelum jam 4 sore.");
        } else {
          // Lanjutkan proses absen jika sudah jam 4 sore atau lebih
          absenButton.disabled = false;
        }
      });
    });
    document.addEventListener("DOMContentLoaded", function() {
  const absenPulangButton = document.getElementById("absenPulangButton");
  const now = new Date();
  const currentHour = now.getHours();

  // Periksa apakah sudah lewat jam 4 sore (16:00)
  if (currentHour >= 16) {
    absenPulangButton.disabled = false; // Aktifkan tombol jika sudah jam 4 sore
  } else {
    absenPulangButton.disabled = true; // Nonaktifkan tombol jika belum jam 4 sore
  }

  // Tambahkan event listener untuk mengecek waktu saat tombol diklik
  absenPulangButton.addEventListener("click", function(event) {
    const clickHour = new Date().getHours();
    if (clickHour < 16) {
      event.preventDefault(); // Cegah form submit jika waktu masih kurang dari 16:00
      alert("Tidak bisa absen pulang sebelum jam 4 sore.");
    }
  });
});
