...
class Penggajian extends BaseController
{
    ...
    public function approveGaji() //berikan message ke bos
    {
        if (session()->get('jabatan') == 'hrd') {
            ...
            $messageForBos = '<a href="/bos/approveBos/' . $bulan . '" >Laporan Gaji Bulan ' . $bulan . ' ' . $tahun . '</a>';
    ...
    public function approveBos($bulan) //berikan message ke karyawan
    {
        // 2023-06-26 23:49:36
        if (session()->get('jabatan') == 'bos') {
            $penggajian = $this->userModel->join('bos', 'bos.id_user=user.id_user')->where('month(tgl)', Time::parse('01-' . $bulan . '-2023', 'Asia/Jakarta', 'id_id')->getMonth())->findAll();
            ...
            // dd($data);
            return view('bos/approveBos', $data);
        } else {
            return redirect()->to('/dashboard');
        }
    }
    ...
    public function beriSlip($tgl)
    {
        ...
        foreach ($id_user as $id) {
            ...
            $messageForKaryawan = '<a href="/bos/slipGaji/' . $tgl . '/' . $id_user . '" >Slip Gaji Bulan ' . Time::parse($tgl, 'Asia/Jakarta', 'id_id')->toLocalizedString('MMMM YYYY') . '</a>';
        ...
        return redirect()->to('/bos/approveBos/' . Time::parse($tgl, 'Asia/Jakarta', 'id_id')->toLocalizedString('MMMM'));
    }

    public function slipGaji($tgl, $id)
    {
        if (session()->get('jabatan') == 'bos' || session()->get('id') == $id) {
            $penggajian = $this->userModel->join('bos', 'bos.id_user=user.id_user')->where('tgl', $tgl)->where('bos.id_user', $id)->first();
